...
export const ChatModule: React.FC<ChatModuleProps> = ({
  ...
  const handleSendQuery = async (queryText?: string, rolePreset?: RolePreset) => {
    ...
    const { citations, contextSnippet } = retrieveRelevantChunks(textToSend, documents, 4);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8.5rem)] min-h-[460px] w-full max-w-5xl mx-auto bg-slate-950 border border-slate-800/80 rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="relative w-full max-w-full bg-slate-900/80 border-b border-slate-800/80 shrink-0 z-10">
        ...
        <div className="absolute left-0 top-0 bottom-0 w-3 bg-gradient-to-r from-slate-900 to-transparent z-10 pointer-events-none sm:hidden" />
        <div className="absolute right-0 top-0 bottom-0 w-3 bg-gradient-to-l from-slate-900 to-transparent z-10 pointer-events-none sm:hidden" />
                        ...
                        return (
                          ...
                          >
                            <div
                              ...
                              className="flex items-center justify-between cursor-pointer text-[11px] sm:text-xs gap-2"
                            >
                            ...
                            {/* Snippet Drawer */}
                            {isExpanded && (
                              <div className="mt-2 pt-2 border-t border-slate-800 text-[10px] sm:text-[11px] text-slate-300 font-mono bg-slate-900/80 p-2 rounded-lg leading-relaxed">
                                <p className="text-[9px] text-slate-500 font-sans mb-1 font-semibold uppercase">Exact Chunk Snippet:</p>
                                "{cite.snippet}"
                              </div>
                            )}
                          </div>
